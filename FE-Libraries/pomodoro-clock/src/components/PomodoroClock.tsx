...
const PomodoroClock: React.FC = () => {
	...
	const [sessionType, setSessionType] = useState<"session" | "break">("session"); // Track session or break type
	...
	const formatTime = (timeInSeconds: number): string => {
	  ...
	  const [intervalId, setIntervalId] = useState<number | null>(null); // Store interval ID
		...
		if (timerRunning) {
		  const newIntervalId = setInterval(() => {
			setTimeLeft((prevTime) => {
			  ...
			  if (prevTime <= 5 && prevTime > 0) { // Log for values close to zero
      ...
      const resetTimer = () => {
        ...
        if (beepSound.current) { // Stop and rewind audio
    ...
    <div className="pomodoro-clock">
      <div className="settings">
        ...
        <div className="setting-group">
		  ...
		  <button id="break-decrement" onClick={decrementBreakLength}>-</button> {/* Added onClick */}
            <span id="break-length">{breakLength}</span>
            <button id="break-increment" onClick={incrementBreakLength}>+</button> {/* Added onClick */}
          </div>
        </div>
        ...
        <div className="setting-group">
          ...
          <div className="controls">
            <button id="session-decrement" onClick={decrementSessionLength}>-</button> {/* Added onClick */}
            <span id="session-length">{sessionLength}</span>
            <button id="session-increment" onClick={incrementSessionLength}>+</button> {/* Added onClick */}
          </div>
        </div>
      </div>
      ...
      <div className="timer">
		...
		<div className="timer-controls">
              ...
              <button id="reset" onClick={resetTimer}>Reset</button> {/* Added onClick */}
            </div>
      </div>
	  <audio id="beep" preload="auto" ref={beepSound} src="https://raw.githubusercontent.com/freeCodeCamp/cdn/master/build/testable-projects-fcc/audio/BeepSound.wav"></audio> {/* Added ref */}
	  <Test13Explanation />
	  </div>
  );
};
